...
static int st21nfc_loc_set_polaritymode(struct st21nfc_device *st21nfc_dev, int mode)
{
				...
				size_t count, loff_t *offset)
{
	...
	if (st21nfc_dev->r_state_current == ST21NFC_HEADER) {
		/* Counting idle index */
		for (idle = 0; idle < ret &&
			st21nfc_dev->buffer[idle] == IDLE_CHARACTER; idle++);
			      ...
			      unsigned int cmd, unsigned long arg)
{
	...
	case ST21NFC_LEGACY_RECOVERY:
		...
		if (!IS_ERR_OR_NULL(st21nfc_dev->gpiod_reset)) {
			if (st21nfc_dev->irq_is_attached) {
				devm_free_irq(&st21nfc_dev->client->dev, st21nfc_dev->client->irq, st21nfc_dev);
				st21nfc_dev->irq_is_attached = false;
			}
			/* pulse low for 20 millisecs */
			gpiod_set_value(st21nfc_dev->gpiod_reset, 0);
	...
	case ST21NFC_USE_ESE:
		...
		if (ret == 0) {
			if (st21nfc_st54spi_cb != 0)
				(*st21nfc_st54spi_cb)(tmp ? ST54SPI_CB_ESE_USED : ST54SPI_CB_ESE_NOT_USED, st21nfc_st54spi_data);
		}
		if (enable_debug_log)
...
static uint64_t st21nfc_power_duration(struct st21nfc_device *data,
				       ...
				       uint64_t current_time_ms)
{
	return data->c_pw_current != pstate ? data->c_pw_states[pstate].duration : data->c_pw_states[pstate].duration + (current_time_ms - data->c_pw_states[pstate].last_entry);
}
				...
				char *buf)
{
	...
	memcpy(data->c_pw_states, data->pw_states, ST21NFC_POWER_STATE_MAX * sizeof(struct nfc_sub_power_stats));
	...
	active_ce_duration = st21nfc_power_duration(data, ST21NFC_ACTIVE, current_time_ms);
	active_rw_duration = st21nfc_power_duration(data, ST21NFC_ACTIVE_RW, current_time_ms);
