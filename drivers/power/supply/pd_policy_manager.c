...
static int usbpd_pps_enable_charging(struct usbpd_pm *pdpm,
...
static bool usbpd_get_pps_status(struct usbpd_pm *pdpm)
{
	...
	while (1) {
		...
		if (apdo_cap.max_mv < pm_config.min_adapter_volt_required || apdo_cap.ma < pm_config.min_adapter_curr_required)
			continue;
		if (apdo_idx == -1) {
			apdo_idx = cap_idx;
...
static int usbpd_pm_enable_sw(struct usbpd_pm *pdpm, bool en)
{
	...
	pr_info("usbpd_pm_enable_sw: en: %d\n", en);
	if (!pdpm->sw_psy) {
		pdpm->sw_psy = power_supply_get_by_name("battery");
		if (!pdpm->sw_psy) {
			return -ENODEV;
...
static void usbpd_check_cp_psy(struct usbpd_pm *pdpm)
{
	if (!pdpm->cp_psy) {
		if (pm_config.cp_sec_enable) {
			...
			pr_info("sc8551-master found\n");
		} else {
			pdpm->cp_psy = power_supply_get_by_name("sc8551-standalone");
			pr_info("sc8551-standalone found\n");
		}

		if (!pdpm->cp_psy)
			pr_info("cp_psy not found\n");
	}
}
...
static void usbpd_pm_update_cp_status(struct usbpd_pm *pdpm)
{
	...
	//pdpm->cp.ibat_curr = pdpm->cp.ibat_curr_sw + pdpm->cp.ibat_curr_cp;
	ret = power_supply_get_property(pdpm->cp_psy,
...
static void usbpd_pm_evaluate_src_caps_maxim(struct usbpd_pm *pdpm)
{
	...
	pr_info("usbpd_pm_evaluate_src_caps: 0x%x\n", (char *)pdpm->pdo);
	...
	for (i = 0; i < 7; i++) {
		...
		if (pdpm->pdo[i].apdo == true) {
			if (pdpm->pdo[i].max_voltage >= pdpm->apdo_max_volt && pdpm->pdo[i].max_current > pdpm->apdo_max_curr) {
				...
				pr_info("[SC manager] vola %d curr %d\n",
...
static void usbpd_pm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	for (i = 0; i < 7; i++) {
		pr_info("[SC manager] %d type %d\n", i, pdpm->pdo[i].type);
		if (pdpm->pdo[i].type == PD_SRC_PDO_TYPE_AUGMENTED && pdpm->pdo[i].pps && pdpm->pdo[i].pos) {
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	/* hardware alarm loop */
	if (pdpm->cp.bat_ocp_alarm || pdpm->cp.bus_ocp_alarm || pdpm->cp.bus_ovp_alarm)
		/*|| pdpm->cp.bat_ovp_alarm*/
	...
	} else if (pdpm->cp.bat_ocp_fault || pdpm->cp.bus_ocp_fault || pdpm->cp.bat_ovp_fault || pdpm->cp.bus_ovp_fault) {
	...
	if ((pdpm->cp.vbat_volt > pm_config.bat_volt_lp_lmt - TAPER_VOL_HYS) && pdpm->cp.ibat_curr < pm_config.fc2_taper_current) {
	...
	if (pdpm->cp.ibat_curr > 0 && ibus_total > 0 && time_delta < QUICK_RAISE_VOLT_INTERVAL_S) {
		...
		pr_info("fcc_ibatt_diff: %d, sicl_ibus_diff: %d\n", fcc_ibatt_diff, sicl_ibus_diff);
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->cp.vbat_volt < pm_config.min_vbat_for_cp) {
			...
			pr_info("pm_sm batt_volt: %d too high, switch main charger\n", pdpm->cp.vbat_volt);
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
		} else if (thermal_level >= MAX_THERMAL_LEVEL_FOR_CP || pdpm->is_temp_out_fc2_range) {
			pr_info("system thermal level too high or batt temp is out of fc2 range, waiting...\n");
		} else {
			pr_info("pm_sm batt_volt: %d is ok, start flash charging\n", pdpm->cp.vbat_volt);
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		pr_info("tune_vbus_retry %d, vbus_low: %d, vbus_high: %d\n", tune_vbus_retry,
				pdpm->cp.vbus_error_low, pdpm->cp.vbus_error_high);
		...
		if (tune_vbus_retry > 30) {
			pr_info("Failed to tune adapter volt into valid range, charge with switching charger, will try to recover\n");
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
	...
	case PD_PM_STATE_FC2_TUNE:
		...
		if (pm_config.cp_sec_enable && pdpm->cp_sec.charge_enabled && pdpm->cp.vbat_volt > pm_config.bat_volt_lp_lmt - 50 &&
...
static void usbpd_pd_contact(struct usbpd_pm *pdpm, bool connected)
{
	...
	pr_info("usbpd_pd_contact pd_active %d\n", pdpm->pd_active);
	if (connected) {
		msleep(10);
		if (NOPMI_CHARGER_IC_MAXIM == nopmi_get_charger_ic_type()) {
		...
		pr_info("start cp charging pps support %d\n", pdpm->pps_supported);
		if (pdpm->pps_supported)
				 ...
				 unsigned long event, void *data)
{
	...
	pr_info("nopmi_init_usb_psy: 0x%x\n", (char *)psy);
...
static int __init usbpd_pm_init(void)
{
	...
	if (!pdpm->cp_psy || (pm_config.cp_sec_enable
		&& !pdpm->cp_sec_psy) || !pdpm->usb_psy) {
		ret = -EPROBE_DEFER;
		if (probe_cnt <= 10)
			return ret;
	}
