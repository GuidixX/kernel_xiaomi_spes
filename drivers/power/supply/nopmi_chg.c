					    ...
					    const union power_supply_propval *val)
{
	...
	if (val->intval < 0 || nopmi_chg->thermal_levels <= 0 || val->intval > nopmi_chg->thermal_levels)
		return -EINVAL;
	...
	if (nopmi_chg->system_temp_level == nopmi_chg->thermal_levels) {
		//thermal temp level
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		...
		break;
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		pr_info("%s: Set input suspend prop, value: %d\n", __func__, val->intval);
			      ...
			      union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		/* set online if usb_online & !low_volt */
		if (g_nopmi_chg->usb_online > 0) {
			val->intval = 1;
			...
			if (nopmi_chg->bms_psy) {
				...
				if (rc < 0) {
					...
					pr_err("%s: get POWER_SUPPLY_PROP_VOLTAGE_NOW fail\n", __func__);
				}
			}
