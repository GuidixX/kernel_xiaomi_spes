...
bool max77729_fg_fuelalert_init(struct max77729_fuelgauge_data *fuelgauge,
				int soc, int volt);
static void max77729_fg_periodic_read_power(struct max77729_fuelgauge_data *fuelgauge);
...
static void max77729_fg_model_load(struct max77729_fuelgauge_data *fg)
{
	...
	if (max77729_read_word(fg->i2c, VEMPTY_REG) != fg->battery_data->V_empty) {
		force_update = true;
	}
...
static void max77729_fg_adaptation_wa(struct max77729_fuelgauge_data *fuelgauge)
{
	...
	if ((rcomp0 > (fg_reset_data->rcomp0 * 14 / 10)) ||
			(rcomp0 < (fg_reset_data->rcomp0 * 7 / 10))) {
	...
	/* check TEMPCO */
	if (max77729_bulk_read(fuelgauge->i2c, TEMPCO_REG, 2, data) < 0) {
...
static int max77729_fg_check_battery_present(struct max77729_fuelgauge_data *fuelgauge)
{
	u8 status_data[2];
	int ret = 1;
					...
					int unit)
{
	...
	if ((vcell < 3500) && (cnt < 10) &&
		(avg_current < 0) && fuelgauge->is_charging) {
		avg_current = 1;
		cnt++;
	}
...
int max77729_fg_reset_capacity_by_jig_connection(struct max77729_fuelgauge_data *fuelgauge)
{
...
static void max77729_fg_read_power_log(struct max77729_fuelgauge_data *fuelgauge)
{
	int vnow, inow;
...
int max77729_fg_alert_init(struct max77729_fuelgauge_data *fuelgauge,
			   int soc, int low_volt_thres)
{
	...
	/* Using RepSOC */
	if (max77729_bulk_read(fuelgauge->i2c, MISCCFG_REG,
			2, misccgf_data) < 0) {
		pr_err("%s: Failed to read MISCCFG_REG\n", __func__);
		return -1;
	}
	...
	if (max77729_bulk_write(fuelgauge->i2c, MISCCFG_REG,
			2, misccgf_data) < 0) {
		pr_info("%s: Failed to write MISCCFG_REG\n", __func__);
		return -1;
	}
	...
	salrt_data[1] = 0xff;
	salrt_data[0] = soc;
	if (max77729_bulk_write(fuelgauge->i2c, SALRT_THRESHOLD_REG,
			2, salrt_data) < 0) {
	...
	talrt_data[1] = 0x7F;
	talrt_data[0] = 0x80;
	if (max77729_bulk_write(fuelgauge->i2c, TALRT_THRESHOLD_REG,
			2, talrt_data) < 0) {
	...
	/* Enable SOC alerts */
	if (max77729_bulk_read(fuelgauge->i2c, CONFIG_REG,
			2, config_data) < 0) {
	...
	if (max77729_bulk_write(fuelgauge->i2c, CONFIG_REG,
			2, config_data) < 0) {
			       ...
			       int enable)
{
	...
	if ((status_data[1] & 0x01) && !fuelgauge->is_charging) {
...
static int max77729_fg_check_capacity_max(struct max77729_fuelgauge_data *fuelgauge,
					  int capacity_max)
{
	int cap_max, cap_min;
...
static int max77729_fg_calculate_dynamic_scale(struct max77729_fuelgauge_data *fuelgauge,
					       int capacity, bool scale_by_full)
{
...
static void max77729_lost_soc_check_trigger_cond(struct max77729_fuelgauge_data *fuelgauge,
						 int raw_soc, int d_raw_soc, int d_remcap, int d_qh)
{
...
static int max77729_lost_soc_calc_soc(struct max77729_fuelgauge_data *fuelgauge,
				      int request_soc, int d_qh, int d_remcap)
{
...
static int max77729_lost_soc_get(struct max77729_fuelgauge_data *fuelgauge,
				 int request_soc)
{
...
static int max77729_fg_check_initialization_result(struct max77729_fuelgauge_data *fuelgauge)
{
	u8 data[2];
				    ...
				    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		switch (val->intval) {
		case SEC_BATTERY_CURRENT_UA:
			val->intval = max77729_fg_read_avg_current(fuelgauge, SEC_BATTERY_CURRENT_UA);
			break;
		case SEC_BATTERY_CURRENT_MA:
		default:
			fuelgauge->current_avg = val->intval = max77729_get_fuelgauge_value(fuelgauge, FG_CURRENT_AVG);
			break;
		}
		break;
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		val->intval = clamp(val->intval, 0, 1000);
		...
		/* makesure divided value is rounded up */
		val->intval = (val->intval > 16) ? ((val->intval * 10 + 99) / 100) : (val->intval / 10);
		...
		if (fuelgauge->shutdown_delay_enable) {
			...
			if (last_shutdown_delay != fuelgauge->shutdown_delay) {
				...
				schedule_delayed_work(&fuelgauge->shutdown_delay_work,
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval = fuelgauge->raw_capacity * (fuelgauge->battery_data->Capacity * fuelgauge->fg_resistor / 2);
		break;
				    ...
				    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_FULL_DESIGN:
		...
		fuelgauge->capacity_max = max77729_fg_check_capacity_max(fuelgauge, val->intval);
		fuelgauge->initial_update_of_soc = true;
		break;
#if defined(CONFIG_BATTERY_AGE_FORECAST)
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		{
			...
			if (max77729_write_word(fuelgauge->i2c, FULLSOCTHR_REG,
				...
				reg_fullsocthr = max77729_read_word(fuelgauge->i2c, FULLSOCTHR_REG);
...
static void max77729_shutdown_delay_work(struct work_struct *work)
{
	...
	psy_do_property("battery", set,
			POWER_SUPPLY_PROP_SHUTDOWN_DELAY, shutdown_val);
}
...
static void max77729_fuelgauge_parse_dt_lost_soc(struct max77729_fuelgauge_data *fuelgauge,
...
static int max77729_fuelgauge_parse_dt(struct max77729_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		if (pdata->jig_gpio >= 0) {
			...
			pdata->jig_low_active = of_property_read_bool(np, "fuelgauge,jig_low_active");
		} else {
