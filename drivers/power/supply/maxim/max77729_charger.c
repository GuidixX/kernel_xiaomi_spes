...
static int max77729_set_fast_charge_mode(struct max77729_charger_data *charger,
					 int pd_active)
{
	int rc = 0;
	...
	if (charger->psy_bms) {
		rc = power_supply_get_property(charger->psy_bms,
				POWER_SUPPLY_PROP_CHIP_OK, &prop);
		if (rc < 0) {
	 ...
	 */
	/* pr_err("%s: batt_verify: %d, batt_soc: %d, batt_temp: %d", __func__, */
...
static int max77729_get_vbus_state(struct max77729_charger_data *charger)
{
	...
	if (is_wireless_type(charger->cable_type))
		...
		reg_data = ((reg_data & MAX77729_CHGIN_DTLS) >> MAX77729_CHGIN_DTLS_SHIFT);
...
static int max77729_get_charging_health(struct max77729_charger_data *charger)
{
	...
	if (charger->is_charging) {
		max77729_read_reg(charger->i2c,
				MAX77729_CHG_REG_DETAILS_00, &reg_data);
	...
	psy_do_property("bms", get, POWER_SUPPLY_EXT_PROP_MEASURE_INPUT, val_iin);

	val_vbyp.intval = SEC_BATTERY_VBYP;
	psy_do_property("bms", get, POWER_SUPPLY_EXT_PROP_MEASURE_INPUT, val_vbyp);
	...
	if (state == POWER_SUPPLY_HEALTH_GOOD && charger->psy_bms) {
		power_supply_get_property(charger->psy_bms,
					   ...
					   *charger, int cable_type)
{
	...
	if (cable_type == SEC_BATTERY_CABLE_WIRELESS) {
		max77729_read_reg(charger->i2c,
				MAX77729_CHG_REG_CNFG_10, &reg_data);
		...
		max77729_read_reg(charger->i2c,
				MAX77729_CHG_REG_CNFG_09, &reg_data);
...
{
	if (is_wireless_type(charger->cable_type))
		return max77729_get_input_current_type(charger,
...
static void reduce_input_current(struct max77729_charger_data *charger, int curr)
{
	...
	input_current = (input_current > max_value) ? max_value :
		((input_current < MINIMUM_INPUT_CURRENT) ? MINIMUM_INPUT_CURRENT :
		 input_current);
...
static void max77729_check_cnfg12_reg(struct max77729_charger_data *charger)
{
	...
	if (is_valid) {
		...
		valid_cnfg12 = (is_wireless_type(charger->cable_type)) ? reg_data :
			(reg_data | (1 << CHG_CNFG_12_CHGINSEL_SHIFT));
...
static void max77729_set_wireless_input_current(struct max77729_charger_data *charger,
						int input_current)
{
	...
	if (is_wireless_type(charger->cable_type)) {
		...
		pr_info("%s: wc_current(%d), wc_pre_current(%d)\n", __func__,
				charger->wc_current, charger->wc_pre_current);
...
static int max77729_set_otg(struct max77729_charger_data *charger, int enable)
{
	...
	if (enable) {
		...
		vote(charger->chgctrl_votable, MAX77729_CHG_VOTER, true,
...
static void max77729_charger_initialize(struct max77729_charger_data *charger)
{
	...
	max77729_write_reg(charger->i2c, MAX77729_CHG_REG_INT_MASK, 0xff); //disable all chg interrupt
				...
				struct device_attribute *attr, char *buf)
{
	...
	case CHIP_ID:
		max77729_read_reg(charger->pmic_i2c,
				MAX77729_PMIC_REG_PMICID1, &data);
					     ...
					     union power_supply_propval *val)
{
	...
	val->intval = !(get_client_vote(charger->usb_icl_votable,
				     ...
				     union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		} else if (get_client_vote_locked(charger->usb_icl_votable,
				MAIN_CHG_ENABLE_VOTER) == MAIN_ICL_MIN) {
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		}
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		val->intval =
			(charger->charge_mode == SEC_BAT_CHG_MODE_CHARGING ? 1 : 0);
		break;
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CHIP_ID:
			if (!max77729_read_reg(charger->i2c,
				...
				val->intval =
					(charger->pmic_ver >= 0x1 &&
					 charger->pmic_ver <= 0x7);
		...
		case POWER_SUPPLY_EXT_PROP_CHARGE_BOOST:
			max77729_read_reg(charger->i2c,
					MAX77729_CHG_REG_CNFG_00, &reg_data);
			reg_data &= CHG_CNFG_00_MODE_MASK;
				     ...
				     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (val->intval == SEC_BATTERY_CABLE_USB ||
			...
			val->intval == SEC_BATTERY_CABLE_TA)
			schedule_delayed_work(&charger->adapter_change_work,
					msecs_to_jiffies(50));
		break;
	case POWER_SUPPLY_PROP_ONLINE:
...
//modify by HTH-234718 at 2022/05/26 end
	...
	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		charger->charge_mode =
			(val->intval ? SEC_BAT_CHG_MODE_CHARGING : SEC_BAT_CHG_MODE_CHARGING_OFF);
		charger->misalign_cnt = 0;
			      ...
			      const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_REAL_TYPE:
		if (charger->real_type != val->intval) {
			...
			schedule_delayed_work(&charger->notify_work,
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		pval->intval =
			!(get_client_vote_locked(charger->usb_icl_votable, MAIN_CHG_ENABLE_VOTER) == MAIN_ICL_MIN);
		break;
	case POWER_SUPPLY_PROP_HEALTH:
	...
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		pval->intval =
			(get_client_vote_locked(charger->usb_icl_votable, MAIN_CHG_SUSPEND_VOTER) == 0);
		break;
		...
		const union power_supply_propval *val)
{
	...
	if (chg->system_temp_level == 0) {
		...
		rc = vote(chg->usb_icl_votable,
				THERMAL_DAEMON_VOTER, (bool)val->intval, 0);
	} else {
...
#endif
			/* print vnow, inow */
			psy_do_property("bms", get, POWER_SUPPLY_EXT_PROP_MONITOR_WORK, value);
		}
	} else {
		max77729_read_reg(charger->i2c, MAX77729_CHG_REG_DETAILS_01, &reg_data);
		reg_data = ((reg_data & MAX77729_BAT_DTLS) >> MAX77729_BAT_DTLS_SHIFT);
...
static void max77729_enable_aicl_irq(struct max77729_charger_data *charger)
{
	...
	ret = request_threaded_irq(charger->irq_aicl, NULL,
			max77729_aicl_irq, 0, "aicl-irq", charger);
	if (ret < 0) {
...
static void max77729_redet_work(struct work_struct *work)
{
	...
	if (!charger->otg_on) {
		if (charger->real_type == POWER_SUPPLY_TYPE_USB_FLOAT ||
...
static void max77729_batt_notify_work(struct work_struct *work)
{
	...
	if (charger->psy_usb) {
		power_supply_get_property(charger->psy_usb,
	...
	if (charger->batt_notify_count++ < 5)
		schedule_delayed_work(&charger->batt_notify_work,
				msecs_to_jiffies(1000));
	else
...
static void max77729_notify_work(struct work_struct *work)
{
	...
	if (charger->psy_usb) {
		...
		power_supply_get_property(charger->psy_usb,
				POWER_SUPPLY_PROP_MTBF_CUR, &pval);
...
static void max77729_adapter_changed_work(struct work_struct *work)
{
	...
	max77729_read_reg(charger->muic, REG_BC_STATUS, &g_usbc_data->muic_data->bc_status);
	g_usbc_data->muic_data->chg_type =
		(g_usbc_data->muic_data->bc_status & BIT_ChgTyp) >> FFS(BIT_ChgTyp);
	g_usbc_data->muic_data->pr_chg_type =
		(g_usbc_data->muic_data->bc_status & BIT_PrChgTyp) >> FFS(BIT_PrChgTyp);

	dcdtmo = (g_usbc_data->muic_data->bc_status & BIT_DCDTmo) >> FFS(BIT_DCDTmo);
	g_usbc_data->muic_data->vbusdet =
		((g_usbc_data->muic_data->bc_status & BIT_VBUSDet) == BIT_VBUSDet) ? 1 : 0;
	...
	case CHGTYP_NOTHING:
		...
		} else if (g_usbc_data->muic_data->vbusdet == 1 && dcdtmo) {
	...
	case CHGTYP_USB_SDP:
		...
		if (g_usbc_data->muic_data->vbusdet == 1 && dcdtmo) {
			...
			real_charger_type = POWER_SUPPLY_TYPE_USB_FLOAT;
			schedule_delayed_work(&charger->redet_work,
	...
	if (ccstat == cc_SOURCE) {
		...
		if (g_usbc_data->is_hvdcp) {
			...
			vote(charger->usb_icl_votable, MAIN_CHG_AICL_VOTER, false, 0);
		} else if (charger->irq_aicl_enabled == 0) {
			charger->irq_aicl_enabled = 1;
			enable_irq(charger->irq_aicl);
		}
	}
...
static void max77729_period_work(struct work_struct *work)
{
	...
	if (charger->psy_bms) {
		...
		rc = power_supply_get_property(charger->psy_bms,
				POWER_SUPPLY_PROP_VOLTAGE_AVG, &pval);
		if (rc < 0) {
			pr_err("%s: get POWER_SUPPLY_PROP_CURRENT_AVG fail\n", __func__);
		}
			      ...
			      int fcc_ma, const char *client)
{
	...
	value.intval = fcc_ma;
	psy_do_property("bbc", set, POWER_SUPPLY_PROP_CURRENT_NOW, value);
			    ...
			    int fv_mv, const char *client)
{
	...
	value.intval = fv_mv;
	psy_do_property("bbc", set, POWER_SUPPLY_PROP_VOLTAGE_MAX, value);
				 ...
				 int icl_ma, const char *client)
{
	...
	value.intval = icl_ma;
	psy_do_property("bbc", set, POWER_SUPPLY_PROP_CURRENT_MAX, value);
...
static int max77729_charger_probe(struct platform_device *pdev)
{
	...
	(void)debugfs_create_file("max77729-regs", S_IRUGO, NULL, (void *)charger, &max77729_debugfs_fops);
	...
	if (charger->pdata->chg_irq) {
		...
		ret = request_threaded_irq(charger->pdata->chg_irq, NULL,
				max77729_chg_irq_thread, 0, "charger-irq", charger);
		if (ret) {
	...
	if (charger->irq_chgin) {
		ret = request_threaded_irq(charger->irq_chgin,
				NULL, max77729_chgin_irq, 0, "chgin-irq", charger);
		if (ret) {
	...
	ret = request_threaded_irq(charger->irq_chgin, NULL,
			max77729_chgin_irq, 0, "chgin-irq", charger);
	if (ret < 0)
	...
	ret = request_threaded_irq(charger->irq_bypass, NULL,
			max77729_bypass_irq, 0, "bypass-irq", charger);
	if (ret < 0)
	...
	ret = request_threaded_irq(charger->irq_batp, NULL,
			max77729_batp_irq, 0, "batp-irq", charger);
	if (ret < 0)
	...
	if ((sec_debug_get_debug_level() & 0x1) == 0x1) {
		...
		ret = request_threaded_irq(charger->irq_bat, NULL,
				max77729_bat_irq, 0, "bat-irq", charger);
		if (ret < 0)
	...
	if (charger->pdata->enable_sysovlo_irq) {
		...
		ret = request_threaded_irq(charger->irq_sysovlo, NULL,
				max77729_sysovlo_irq, 0, "sysovlo-irq", charger);
		if (ret < 0)
