...
#define ONE_WIRE_CONFIG_OUT		\
	writel_relaxed(GPIO_ENABLE | DRV_STRENGTH_16MA | GPIO_OUTPUT | GPIO_PULL_UP, \
			g_onewire_data->gpio_cfg_reg) // OUT
#define ONE_WIRE_CONFIG_IN		\
	writel_relaxed(GPIO_ENABLE | DRV_STRENGTH_16MA | GPIO_INPUT | GPIO_PULL_UP, \
...
void write_byte(char val)
{
	...
	raw_spin_lock_irqsave(&g_onewire_data->lock, flags);
	ONE_WIRE_CONFIG_OUT; // writes byte, one bit at a time
	for (i = 0; i < 8; i++) {
