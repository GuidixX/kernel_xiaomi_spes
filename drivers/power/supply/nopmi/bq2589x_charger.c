...
static int bq2589x_set_fast_charge_mode(struct bq2589x *bq, int pd_active)
{
	...
	/*If TA plug in with PPS, battery auth success and soc less than 95%,
	 * FFC flag will enabled. The temp is normal set fastcharge mode as 1
	 * and jeita loop also handle fastcharge prop*/
	...
	if ((pd_active == 2) && batt_soc < 95) {
		...
		/*If TA plug in without PPS, battery auth fail and soc exceed 95%,
		 * FFC will always be disabled*/
		propval.intval = 0;
		g_ffc_disable = true;
	}
...
//modify by HTH-209427/HTH-209841 at 2022/05/12 end
	...
	if (ret < 0) {
		pr_err("%s: Failed to set vindpm offset: %d\n", __func__, ret);
		return ret;
	}
	...
	if (ret < 0) {
		pr_err("%s: Failed to set charge voltage: %d\n", __func__, ret);
		return ret;
	}
	...
	if (ret) {
		pr_err("%s: Failed to enable pumpx: %d\n", __func__, ret);
		return ret;
	}
...
static int bq2589x_psy_register(struct bq2589x *bq)
{
	...
	if (IS_ERR(bq->wall_psy)) {
		...
		pr_err("%s: failed to register wall psy: %d\n", __func__, ret);
	}
...
{
	pr_info("%s: stage unregister wall psy\n", __func__);
}
...
static int bq2589x_parse_dt(struct device *dev, struct bq2589x *bq)
{
	...
	if (ret < 0) {
		...
		bq_dbg(PR_OEM, "%s intr_gpio info %d\n", __func__, bq->irq_gpio);
	}
...
static void bq2589x_adjust_absolute_vindpm(struct bq2589x *bq)
{
	...
	if (ret < 0)
		pr_err("Set absolute vindpm threshold %d Failed: %d\n", vindpm_volt, ret);
	else
...
int main_get_charge_type(void)
{
	...
	type = g_bq->vbus_type;
	return (int)type;
	//bq2589x_get_vbus_type(g_bq); 2021.09.11 wsy edit for crash
}
	...
	case BQ2589X_VBUS_USB_SDP:
		...
		if (bq->usb_psy) {
			ret = power_supply_get_property(bq->usb_psy,
					POWER_SUPPLY_PROP_MTBF_CUR, &propval);
			if (ret < 0) {
...
//modify by HTH-209427/HTH-209841 at 2022/05/12 begin
	...
	if (ret < 0)
		pr_err("%s: reset vindpm threshold to 4600 failed: %d\n", __func__, ret);
	else
...
static void bq2589x_monitor_workfunc(struct work_struct *work)
{
	...
	if (bq->bms_psy) {
		...
		pr_info("%s: get battery voltage_now: %d\n", __func__, batt_voltage_now);
	}
	if (batt_temp > 480) {
		...
		if (batt_voltage_now <= 4300) {
			pr_info("%s: high temp: batt_voltage_now <= 4300: enable chargring\n", __func__, batt_voltage_now);
				    ...
				    enum power_supply_type chg_type)
{
	...
	propval.intval = (chg_type != POWER_SUPPLY_TYPE_UNKNOWN);
	...
	if (bq->pd_active && (propval.intval == false)) {
		// Do Nothing for fix CtoC disconnection
	} else {
			    ...
			    int fv_mv, const char *client)
{
	...
	if (rc < 0) {
		pr_err("failed to set charge voltage\n");
		return rc;
	}
