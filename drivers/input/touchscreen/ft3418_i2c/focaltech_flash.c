...
***********************************************************************/
static bool fts_fwupg_check_flash_status(u16 flash_status,
...
static int fts_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
	...
	FTS_INFO("fw paramcfg version offset:%x", upg->func->paramcfgveroff); *ver = upg->fw[upg->func->paramcfgveroff];
...
static int fts_param_ide_in_host(struct fts_upgrade *upg)
{
	...
	if (upg->fw_length < upg->func->paramcfgoff + FTS_FW_IDE_SIG_LEN) {
		FTS_INFO("fw len(%x) < paramcfg offset(%x), no IDE", upg->fw_length, upg->func->paramcfgoff + FTS_FW_IDE_SIG_LEN);
		return 0;
	}
...
static int fts_fwupg_get_vendorid(struct fts_upgrade *upg, int *vid)
{
	...
	if (fwvalid) {
		...
		if ((cfgbuf[FTS_CONIFG_VENDORID_OFF] + cfgbuf[FTS_CONIFG_VENDORID_OFF + 1]) == 0xFF)
