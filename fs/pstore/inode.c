...
{
	return simple_read_from_buffer(buf, len, offset, console_buffer, console_bufsize);
}
...
#ifdef CONFIG_PSTORE_LAST_KMSG
	last_kmsg_entry = proc_create_data("last_kmsg", S_IFREG | S_IRUGO, NULL, &last_kmsg_fops, NULL);
	if (!last_kmsg_entry)
