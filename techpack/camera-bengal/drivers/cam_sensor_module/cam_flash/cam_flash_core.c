...
/* Spes flashlight by muralivijay@github */
	...
	CAM_INFO(CAM_FLASH,
	"Flash off Triggered flash_now %d with value 0 and flash_en %d with value 0",
	mgpio_flash_led.flash_now, mgpio_flash_led.flash_en);
#endif
	return 0;
}
...
/* Spes flashlight by muralivijay@github */
#ifdef CONFIG_CAMERA_FLASH_SPES
	CAM_INFO(CAM_FLASH, "Flash low Triggered flash_now %d with value 1",
			mgpio_flash_led.flash_now);
	rc = cam_res_mgr_gpio_request(soc_info.dev, mgpio_flash_led.flash_now,
			0, "CUSTOM_GPIO1");
	if (rc) {
...
#ifdef CONFIG_CAMERA_FLASH_SPES
	CAM_INFO(CAM_FLASH, "Flash high Triggered flash_en %d with value 1",
			mgpio_flash_led.flash_en);
	rc = cam_res_mgr_gpio_request(soc_info.dev, mgpio_flash_led.flash_en,
			0, "CUSTOM_GPIO1");
	if (rc) {
		CAM_ERR(CAM_FLASH, "gpio %d request fails", rc);
		return rc;
